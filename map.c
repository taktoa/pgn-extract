...
static short Knight_moves[2 * NUM_KNIGHT_MOVES] = {1, 2,
    1, -2,
    2, 1,
    2, -1,
    -1, 2,
...
static short Bishop_moves[2 * NUM_BISHOP_MOVES] = {1, 1,
    1, -1,
    -1, 1,
    -1, -1};
...
static short Rook_moves[2 * NUM_ROOK_MOVES] = {1, 0,
    0, 1,
    -1, 0,
    0, -1};
...
static short King_moves[2 * NUM_KING_MOVES] = {1, 0,
    1, 1,
    1, -1,
    0, 1,
    0, -1,
...
static short Queen_moves[2 * NUM_QUEEN_MOVES] = {1, 0,
    1, 1,
    1, -1,
    0, 1,
    0, -1,
...
{ /* Assume failure. */
    ...
    short king_c = ColConvert(king_col);
    short king_r = RankConvert(king_rank);
    /* The king's destination column. */
    short king_final_c = castling == KINGSIDE_CASTLE ? ColConvert('g') : ColConvert('c');

    short rook_c = ColConvert(rook_col);
    short rook_final_c = castling == KINGSIDE_CASTLE ? ColConvert('f') : ColConvert('d');

    if (king_final_c != king_c) {
        /* Check for clear spaces for the king. */
        short direction = king_final_c > king_c ? 1 : -1;
        short next_c = king_c + direction;
    ...
    if (Ok && rook_final_c != rook_c) {
        ...
        short direction = rook_final_c > rook_c ? 1 : -1;
        short next_c = rook_c + direction;
        Boolean check_again = TRUE;
        while (Ok && check_again) {
